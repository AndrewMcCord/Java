...
/* 	This next functions sets the parameters for what the user will input and the data that will be collected
            @param: none
            @return: none
*/
 public void FlightPlanning(){
	...
	Call function output(plane, a1)
}

/*	this method calculates the actual distance between a starting and destination airplane
...
double distance;
    		...
    		// use the following formula to calculate the distance between the two stations by the latitude and longitude given;
   		 distance = Math.sqrt(R * (Math.pow(Math.sin(lat2) - Math.sin(lat1), 2) + Math.pow(Math.cos(lat2), 2) + Math.pow(Math.cos(lat1), 2) - 2 * Math.cos(lat1) * Math.cos(lat2) * Math.cos(long2 - long1)));
    		return distance;
}
    	...
    	If (angle is between 0 and 180) then {
       	 ...
       	 If (the fuel_type is the same as that of rule of the airplane) then {
           	 If (Both fuelEnough(w, a, s1.getLatitude(), s1.getLongitude(),
               	 temp.getLatitude(), temp.getLongitude()) and fuelEnough(w, a,
...
/*
this method is to find the maximum distance that an Airplaneplane ap can fly without refilling its tank.

@param ap is the Airplane
@return a double value to represent the maximum distance in miles that Airplane ap can fly.
*/
...
/*	this calculates the actual direction a plane must travel between a starting and ending airport,
...
Public double calculateHeading(double airspeed, double lat1, double long1, double lat2, double long2){
	Double angle;
	Let angle be the result of the formula: 
Heading=arctan⁡(sin⁡(Δlong)⋅cos⁡(lat2)/cos⁡(lat1)⋅sin⁡(lat2)−sin⁡(lat1)⋅cos⁡(lat2)⋅cos⁡(Δlong))
	Return angle;
}
...
Construct an empty UserDatabase for userDbase;
}
...
public void search() {
        Insert GUI dropdown menu displaying airplane elements;
        Insert GUI textbox for manual search input;
        send query to database;
        summarize closest matching element selected between dropdown and text input;
        Send summary to variable x;
